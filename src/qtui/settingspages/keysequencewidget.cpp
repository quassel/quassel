    ...
    if (_isRecording) {
        if (_modifierKeys) {
            ...
            if (_modifierKeys & Qt::META)
                s += QChar(static_cast<int>(kControlUnicode));
            if (_modifierKeys & Qt::ALT)
                s += QChar(static_cast<int>(kOptionUnicode));
            if (_modifierKeys & Qt::SHIFT)
                s += QChar(static_cast<int>(kShiftUnicode));
            if (_modifierKeys & Qt::CTRL)
                s += QChar(static_cast<int>(kCommandUnicode));
#else
            if (_modifierKeys & Qt::META)
                s += tr("Meta", "Meta key") + '+';
            if (_modifierKeys & Qt::CTRL)
