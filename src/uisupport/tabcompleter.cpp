...
void TabCompleter::buildCompletionList()
{
    ...
    QRegularExpression sectionRx(R"([^-_\[\]{}|`^.\\#\w\d])");
    Q_ASSERT(sectionRx.isValid());
    QString tabAbbrev = _lineEdit->text().left(_lineEdit->cursorPosition()).section(sectionRx, -1, -1);
...
bool TabCompleter::eventFilter(QObject* obj, QEvent* event)
{
    ...
    if (shortcut.count() > 0) {
        QKeyCombination keyComb = shortcut[0];
        if (QKeyCombination(keyEvent->modifiers(), Qt::Key(keyEvent->key())) == keyComb)
            complete();
        else
            reset();
    } else {
