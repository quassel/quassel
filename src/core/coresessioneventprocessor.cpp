...
{
    if (e->testFlag(EventManager::Fake))
        return;
    ...
    if (net->enabledCaps().contains(IrcCap::EXTENDED_JOIN)) {
        if (e->params().count() < 3) {
            qDebug() << "extended-join requires 3 params, got:" << e->params() << ", handling as a regular join";
        }
        else {
    ...
    if (net->isMe(ircuser)) {
        ...
        net->putRawLine(net->serverEncode("MODE " + channel));
    }
}
...
void CoreSessionEventProcessor::processIrcEventMode(IrcEvent* e)
{
    ...
    if (e->network()->isChannelName(e->params().first())) {
        ...
        if (!channel) {
            // We received mode information for a channel we're not in. Discard the data.
            return;
        }
...
void CoreSessionEventProcessor::processIrcEvent005(IrcEvent* e)
{
    ...
    for (int i = 0; i < e->params().count() - 1; i++) {
        ...
        if (key == "PREFIX") {
            // Example: PREFIX=(ov)@+
            if (value.startsWith('(')) {
                modes = value.section(')', 0, 0).mid(1);  // Extract modes (e.g., "ov")
                prefixes = value.section(')', 1, 1);      // Extract prefixes (e.g., "@+")
            }
            else {
                // Default prefixes if not specified
                modes = "ov";
                prefixes = "@+";
            }
            e->network()->setPrefixes(modes, prefixes);
        }
        else {
...
{
    IrcUser* me = coreNetwork(e)->ircUser(coreNetwork(e)->myNick());
    if (me)
        me->setAway(false);

    if (e->network()->autoAwayActive()) {
        e->network()->updateAutoAway(false);
        e->setFlag(EventManager::Silent);
    }
}
...
void CoreSessionEventProcessor::processIrcEvent353(IrcEvent* e)
{
    ...
    bool _useCapMultiPrefix = coreNetwork(e)->enabledCaps().contains(IrcCap::MULTI_PREFIX);
                                                      ...
                                                      const QString& realname)
{
    ...
    if (net->enabledCaps().contains(IrcCap::MULTI_PREFIX)) {
    ...
    Network* net, const QString& channel, const QStringList& users, const QStringList& modes, const QString& quitMessage)
{
    ...
    for (const IrcUser* user : ircUsers) {
        nickList.append(user->nick());
    }
    ircChannel->joinIrcUsers(nickList, newModes);
...
void CoreSessionEventProcessor::handleEarlyNetsplitJoin(Network* net, const QString& channel, const QStringList& users, const QStringList& modes)
{
    ...
    for (NetworkEvent* event : events) {
        event->setFlag(EventManager::Fake);
        emit newEvent(event);
    }
}
